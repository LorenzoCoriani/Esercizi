		...
		try{
			do{
				...
				switch (risposta){
					...
					// contains non funziona con i vector e gli oggetti custom perche' non chiama il metodo equals
					//segmentoTrovato = v.contains(strova);
					
					for(int i=0;i<v.size();i++){
						s=(Segmento)v.get(i);
						
						if(s.p.x==strova.p.x && s.p.y==strova.p.y && s.p2.x==strova.p2.x && s.p2.y==strova.p2.y){
							segmentoTrovato=true;
						}				
					}
